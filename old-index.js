...
function startSound(e) {
    ...
    if(Object.keys(keyboard).includes(input) && !keyboard[input].down) {
        oscillators.forEach(osc => {
            ...
            document.addEventListener("keyup", e => stopSound(gainNode, oscNode, input, osc))
        })
...
function stopSound(gainNode, oscNode, input, osc) {
    ...
    gainNode.gain.exponentialRampToValueAtTime(0.0000000001, audioContext.currentTime + parseFloat(releaseTime))

    setTimeout(() => {
        gainNode.disconnect()
        oscNode.disconnect()
        // console.log("at stop:", nodes)
    }, 1000 * releaseTime + 1000)
